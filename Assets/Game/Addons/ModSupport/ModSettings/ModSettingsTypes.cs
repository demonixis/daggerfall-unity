...
namespace DaggerfallWorkshop.Game.Utility.ModSupport.ModSettings
{
    ...
    public class TupleIntKey : Key<Tuple<int, int>>
    {
        ...
        public override int OnEditorWindow(Rect rect, HorizontalCallback horizontal, VerticalCallback vertical, Dictionary<string, object> cache)
        {
            if (Value == null) Value = Tuple.Create(0, 100);
            horizontal(rect, "Value",
                (r) => Value = Tuple.Create(EditorGUI.IntField(r, "First", Value.Item1), Value.Item2),
                (r) => Value = Tuple.Create(Value.Item1, EditorGUI.IntField(r, "Second", Value.Item2)));
            return 1;
        }
#endif
    ...
    public class TupleFloatKey : Key<Tuple<float, float>>
    {
        ...
        public override int OnEditorWindow(Rect rect, HorizontalCallback horizontal, VerticalCallback vertical, Dictionary<string, object> cache)
        {
            if (Value == null) Value = Tuple.Create(0.0f, 100.0f);
            horizontal(rect, "Value",
                (r) => Value = Tuple.Create(EditorGUI.FloatField(r, "First", Value.Item1), Value.Item2),
                (r) => Value = Tuple.Create(Value.Item1, EditorGUI.FloatField(r, "Second", Value.Item2)));
            return 1;
        }
#endif
